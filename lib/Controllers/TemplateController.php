...
class TemplateController
{
    ...
    {
        $video = "<video id='home-video' class='hero-video' loop muted autoplay playsinline " . self::_getPosterImage() . ">";
    ...
    public static function getPageHeader()
    {
        ...
        if (is_search()) {
            ...
            if (is_singular('staff')) {
                ...
                $bkgd = get_post_meta($post->ID, "_page_header_image_id") ? wp_get_attachment_image_url(get_post_meta($post->ID, "_page_header_image_id")[0], "large") : get_the_post_thumbnail_url(null, "large");
                if (!$bkgd) {
                    $bkgd = get_field('default_featured_image', 'options');
                }
            }
    ...
    public static function generateThemeJson()
    {
        ...
        file_put_contents(get_stylesheet_directory() . "/theme.json", wp_json_encode($json), LOCK_EX);
    }
}
